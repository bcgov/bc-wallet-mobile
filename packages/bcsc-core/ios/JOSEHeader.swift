...
enum PublicServerKeyState {
  static var serverPublicKey: SecKey?
  static var serverPublicKeyId: String?
}
...
class JOSEHeader {
  ...
  init(header: [String: String], encoded: String) {
    self.header = header
...
class JWSHeader: JOSEHeader {
  ...
  init(alg: JWSAlgorithm, header: [String: String], encoded: String) {
    self.alg = alg
    super.init(header: header, encoded: encoded)
  }

  init(alg: JWSAlgorithm, header: [String: String]) {
    self.alg = alg
  ...
  class func parse(_ s: String) throws -> JWSHeader {
    if let header: [String: String] = try JSONSerialization.jsonObject(
      with: Base64URL.decode(s),
      options: .mutableContainers
    ) as? [String: String] {
      return JWSHeader(alg: JWSAlgorithm.parse(header["alg"]!), header: header, encoded: s)
    } else {
...
class JWEHeader: JOSEHeader {
  ...
  init(alg: JWEAlgorithm, enc: EncryptionMethod, header: [String: String], encoded: String) {
    self.alg = alg
    self.enc = enc
    self.cty = "JWT"
    self.kid = PublicServerKeyState.serverPublicKeyId ?? ""
  ...
  static func parse(_ s: String) throws -> JWEHeader {
    if let header: [String: String] = try JSONSerialization.jsonObject(
      ...
      return try JWEHeader(
        alg: JWEAlgorithm.parse(s: header["alg"]!),
        enc: EncryptionMethod.parse(header["enc"]!),
        header: header,
        encoded: s
      )
    } else {
