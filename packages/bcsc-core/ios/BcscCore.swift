...
class BcscCore: NSObject {
  ...
  func setToken(
    ...
    if let expiryTimestamp = expiry {
      expiryDate = Date(timeIntervalSince1970: expiryTimestamp.doubleValue)
    }
    ...
    if success {
      logger.log("setToken: Successfully saved token of type \(tokenType)")
      resolve(true)
    } else {
  ...
  func setPIN(
    _ accountID: String, pin: String, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard !accountID.isEmpty, !pin.isEmpty else {
      reject("E_INVALID_PARAMETERS", "AccountID and PIN cannot be empty", nil)
      return
    }
    ...
    guard account.id == accountID else {
      reject("E_ACCOUNT_MISMATCH", "Account ID does not match current account", nil)
      return
    }
    ...
    do {
      let walletKey = try pinService.setPIN(issuer: account.issuer, accountID: account.id, pin: pin, isAutoGenerated: false)
      resolve([
        "success": true,
        "walletKey": walletKey,
  ...
  func deletePIN(
    _ accountID: String, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard !accountID.isEmpty else {
      reject("E_INVALID_PARAMETERS", "AccountID cannot be empty", nil)
      return
    }
  ...
  func setAccountSecurityMethod(
    _ accountID: String, securityMethod: String, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard !accountID.isEmpty, !securityMethod.isEmpty else {
      reject("E_INVALID_PARAMETERS", "AccountID and securityMethod cannot be empty", nil)
      return
    }
  ...
  func unlockWithDeviceSecurity(
    _ accountID: String, reason: String?, resolve: @escaping RCTPromiseResolveBlock,
    reject: @escaping RCTPromiseRejectBlock
  ) {
    guard !accountID.isEmpty else {
      reject("E_INVALID_PARAMETERS", "AccountID cannot be empty", nil)
    ...
    Task {
      let authSuccess = await deviceAuthService.performAuthentication(reason: authReason)
        ...
        if authSuccess {
          ...
          if let hashResult = pinService.getPINHash(issuer: account.issuer, accountID: account.id) {
            ...
            do {
              ...
              let hash = try pinService.setPIN(issuer: account.issuer, accountID: account.id, pin: pin, isAutoGenerated: true)
              resolve([
                "success": true,
                "walletKey": hash,
                "migrated": true
  ...
  /// This reads from the authorization_request file in Application Support,
  ...
  func getCredential(
    ...
    do {
      ...
      guard let clientRegistration = unarchiver.decodeObject(of: ClientRegistration.self, forKey: NSKeyedArchiveRootObjectKey),
            let credential = clientRegistration.credential else {
  ...
  func setCredential(
    ...
    do {
      ...
      if FileManager.default.fileExists(atPath: fileUrl.path) {
        ...
        clientRegistration = unarchiver.decodeObject(of: ClientRegistration.self, forKey: NSKeyedArchiveRootObjectKey) ?? ClientRegistration()
      } else {
      ...
      guard let issuer = credentialData["issuer"] as? String,
            ...
            let bcscReason = credentialData["bcscReason"] as? String else {
  ...
  func deleteCredential(
    ...
    do {
      ...
      guard let clientRegistration = unarchiver.decodeObject(of: ClientRegistration.self, forKey: NSKeyedArchiveRootObjectKey) else {
