...
class BcscCore: NSObject {
  ...
  func unlockWithDeviceSecurity(
    ...
    logger.log("unlockWithDeviceSecurity: env=\(storage.issuer), account=\(account.id)")
    ...
    Task { @MainActor in
      ...
      if authSuccess {
        ...
        if let hashResult = pinService.getPINHash(issuer: account.issuer, accountID: account.id) {
          ...
          logger.log("unlockWithDeviceSecurity: No PIN hash found — triggering V3 migration for account \(account.id)")
          do {
            ...
            logger.log("unlockWithDeviceSecurity: V3 migration complete for account \(account.id)")
            resolve([
              "success": true,
              "walletKey": hash,
              "migrated": true,
