...
class BcscCore: NSObject {
    ...
    func getKeyPair(
        ...
        do {
            ...
            guard
                let publicKeyData = SecKeyCopyExternalRepresentation(keyPair.public, &error)
                    as Data?
            else {
                // Handle error, maybe reject the promise
                let nsError = error!.takeRetainedValue() as Error
                reject(
                    "E_KEY_EXPORT", "Failed to export public key: \(nsError.localizedDescription)",
                    nsError)
            ...
            guard
                let privateKeyData = SecKeyCopyExternalRepresentation(keyPair.private, &error)
                    as Data?
            else {
                // Handle error, maybe reject the promise
                let nsError = error!.takeRetainedValue() as Error
                reject(
                    "E_KEY_EXPORT", "Failed to export private key: \(nsError.localizedDescription)",
                    nsError)
            ...
            reject(
                "E_UNKNOWN", "An unexpected error occurred: \(error.localizedDescription)", error)
        }
    }
    ...
    func setAccount(
        ...
        guard let issuer = account["issuer"] as? String,
            let clientID = account["clientID"] as? String,
    ...
    func signPairingCode(
        _ code: String, issuer: String, clientID: String, fcmDeviceToken: String,
        deviceToken: String?,
        resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock
    ) {
        ...
        addDeviceInfoClaims(
            to: builder, fcmDeviceToken: fcmDeviceToken, deviceToken: actualDeviceToken)
    ...
    func getDynamicClientRegistrationBody(
        _ fcmDeviceToken: NSString, deviceToken: NSString?,
        resolve: @escaping RCTPromiseResolveBlock,
        reject: @escaping RCTPromiseRejectBlock
    ) {
        let keyPairManager = KeyPairManager()
        ...
        guard let keyData = RSAUtil.secKeyRefToData(inputKey: keyPair.public) else {
            reject(
                "E_KEY_DATA_EXTRACTION_FAILED", "Failed to extract key data from public key", nil)
            return
        }

        guard let (modulus, exponent) = RSAUtil.splitIntoComponents(keyData: keyData) else {
            reject(
                "E_KEY_COMPONENT_PARSING_FAILED", "Failed to parse RSA components from key data",
                nil)
            return
        }
        ...
        // Add device info claims using consolidated method
        addDeviceInfoClaims(
            to: builder, fcmDeviceToken: fcmDeviceToken as String,
        ...
        guard let deviceInfoJWTAsString = try? deviceInfoJWT.serialize() else {
            reject(
                "E_DEVICE_INFO_JWT_CONVERSION_FAILED",
                "Failed to convert device info JWT to JSON string",
                nil)
            return
        }
        ...
        do {
            ...
            let jsonData = try JSONSerialization.data(
                withJSONObject: clientRegistrationData, options: [])
    ...
    func getDeviceCodeRequestBody(
        ...
        // Validate all parameters are provided
        guard !deviceCode.isEmpty, !clientID.isEmpty, !issuer.isEmpty, !confirmationCode.isEmpty
        else {
            reject(
                "E_INVALID_PARAMETERS",
        ...
        guard
            let serializedJWT = createClientAssertionJWT(
                audience: issuer, issuer: clientID, subject: clientID,
    ...
    func createQuickLoginJWT(
        ...
        guard
            !accessToken.isEmpty && !clientId.isEmpty && !issuer.isEmpty && !clientRefId.isEmpty
    ...
    private func makeJWTPayloadForAccountLogin(
        ...
        if !addDeviceInfoClaims(
            to: builder, fcmDeviceToken: fcmDeviceToken, deviceToken: deviceToken)
        {
            return nil
        }
    ...
    private func assertionFactoryHMAC(accessToken: String, jwtID: String, clientID: String)
        -> String
    {
        ...
        CCHmacInit(
            &ctx, CCHmacAlgorithm(kCCHmacAlgSHA256), accessTokenBytes, accessTokenBytes.count)
    ...
    private func encryptJWTWithPublicKey(
        serializedJWT: String, publicKey: SecKey, reject: @escaping RCTPromiseRejectBlock
    ) throws -> String? {
        do {
            let jwe = try JWE(
