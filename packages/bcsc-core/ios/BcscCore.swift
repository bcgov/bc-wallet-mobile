...
class BcscCore: NSObject {
  ...
  func getToken(
    ...
    if let token = tokenStorageService.get(id: id) {
      ...
      logger.log("[MIGRATION] getToken: NOT found type=\(tokenType.rawValue) id=\(id)")
      resolve(nil)
    }
  }
  ...
  func unlockWithDeviceSecurity(
    ...
    logger.log("[MIGRATION] unlockWithDeviceSecurity: env=\(storage.issuer), account=\(account.id)")
    ...
    Task { @MainActor in
      ...
      if authSuccess {
        ...
        if let hashResult = pinService.getPINHash(issuer: account.issuer, accountID: account.id) {
          ...
          logger.log("[MIGRATION] unlockWithDeviceSecurity: No PIN hash found — triggering V3 migration for account \(account.id)")
          do {
            ...
            logger.log("[MIGRATION] unlockWithDeviceSecurity: V3 migration complete for account \(account.id)")
            resolve([
              "success": true,
              "walletKey": hash,
              "migrated": true,
