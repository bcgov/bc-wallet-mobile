...
class RSAEncrypter: JWEEncrypter {
  ...
  func encrypt(jwe: JWE, clearText: [UInt8]) throws {
    ...
    if let cipherText = try AESCBC.encryptAuthenticated(
      encKey: compositeKey.encKey,
      iv: iv,
      clearText: clearText,
      aad: aad
    ) {
      // Do MAC
      let hmac = HMAC.compute(
        macKey: compositeKey.macKey.arrayOfBytes(),
        aad: aad,
        iv: iv,
        e: cipherText.arrayOfBytes()
      )
...
class CompositeKey {
  ...
  init(_ secretKey: Data) throws {
    ...
    if secretKey.count == 32 {
      ...
      throw JOSEException(
        "Unsupported AES/CBC/PKCS5Padding/HMAC-SHA2 key length, must be 256, 384 or 512 bits, but is \(secretKey.count)"
      )
    }
  }
}
...
class AESCBC {
  ...
  class func encryptAuthenticated(encKey: Data, iv: [UInt8], clearText: [UInt8], aad _: [UInt8]) throws -> Data? {
...
public enum ValidKeySize {
  ...
  func isValid(keySize: Int) -> Bool {
    switch self {
    case let .fixed(fixed): return fixed == keySize
    case let .range(min, max): return (keySize >= min) && (keySize <= max)
    case let .discrete(values): return values.contains(keySize)
    }
  }
  ...
  func padded(keySize: Int) -> Int? {
    ...
    case let .discrete(values):
      return values.sorted().reduce(nil) { answer, current in
        answer ?? ((current >= keySize) ? current : nil)
      }
    }
  }
}
...
class Cryptor {
  ...
  convenience init(operation: Operation, algorithm: Algorithm, padding: Int, key: [UInt8], iv: [UInt8]) {
    ...
    self.init(
      ...
      algorithm: algorithm,
      padding: padding,
      keyBuffer: zeroPad(array: key, blockSize: paddedKeySize),
  ...
  func final() -> [UInt8]? {
    ...
    var dataOut = [UInt8](repeating: 0, count: byteCount)
    var dataOutMoved = 0
   ...
   */
  @discardableResult open func final(
    bufferOut: UnsafeMutableRawPointer,
    byteCapacityOut: Int,
  ...
  @discardableResult open func update(buffer: UnsafeRawPointer, byteCount: Int) -> Self? {
    ...
    var dataOut = [UInt8](repeating: 0, count: outputLength)
    var dataOutMoved = 0
    update(
      bufferIn: buffer,
      byteCountIn: byteCount,
      bufferOut: &dataOut,
      byteCapacityOut: dataOut.count,
      byteCountOut: &dataOutMoved
    )
    if self.status != Status.success {
      return nil
    }
  ...
  /**
   ...
   - returns:
   */
  @discardableResult open func update(
    bufferIn: UnsafeRawPointer,
    byteCountIn: Int,
    bufferOut: UnsafeMutableRawPointer,
    byteCapacityOut: Int,
    byteCountOut: inout Int
...
public func zeroPad(array: [UInt8], blockSize: Int) -> [UInt8] {
  ...
  return array + [UInt8](repeating: 0, count: pad)
}
