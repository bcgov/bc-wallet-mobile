...
class RSAUtil {
  ...
  class func generateKeyPair(pubLabel _: CFString, privLabel _: CFString) -> (publicKey: SecKey, privateKey: SecKey) {
    let publicKeyAttributes: NSDictionary = [kSecAttrLabel: publicKeyLabel, kSecAttrIsPermanent: kCFBooleanTrue]
    let privateKeyAttributes: NSDictionary = [
  ...
  class func insertPublicKey(
    ...
    var publicAttributes = [String: AnyObject]()
  ...
  class func splitIntoComponents(keyData: Data) -> (modulus: Data, exponent: Data)? {
    ...
    let keyBytes = [CUnsignedChar](UnsafeBufferPointer<CUnsignedChar>(
      start: pointer,
      count: keyData.count / MemoryLayout<CUnsignedChar>.size
    ))
    ...
    if let _ = NSInteger(octetBytes: keyBytes, startIdx: &i) {
      ...
      if keyBytes[i] == 0x02 {
        ...
        if let modulusLength = NSInteger(octetBytes: keyBytes, startIdx: &i) {
          ...
          let modulus = keyData.subdata(in: Range(NSRange(location: i, length: modulusLength))!)
          // var k = Range.i
          i += modulusLength
          ...
          if keyBytes[i] == 0x02 {
            ...
            if let exponentLength = NSInteger(octetBytes: keyBytes, startIdx: &i) {
              let exponent = keyData.subdata(in: Range(NSRange(location: i, length: exponentLength))!)
              i += exponentLength
...
private extension Data {
  func splitIntoComponents() -> (modulus: Data, exponent: Data)? {
    ...
    let keyBytes = [CUnsignedChar](UnsafeBufferPointer<CUnsignedChar>(
      start: pointer,
      count: self.count / MemoryLayout<CUnsignedChar>.size
    ))
    ...
    if let _ = NSInteger(octetBytes: keyBytes, startIdx: &i) {
      ...
      if keyBytes[i] == 0x02 {
        i += 1
        if let modulusLength = NSInteger(octetBytes: keyBytes, startIdx: &i) {
          let modulus = self.subdata(in: Range(NSRange(location: i, length: modulusLength))!)
          i += modulusLength
          ...
          if keyBytes[i] == 0x02 {
            i += 1
            if let exponentLength = NSInteger(octetBytes: keyBytes, startIdx: &i) {
              let exponent = self.subdata(in: Range(NSRange(location: i, length: exponentLength))!)
              i += exponentLength
