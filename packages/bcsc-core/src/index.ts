...
export const setToken = async (tokenType: TokenType, token: string, expiry?: number): Promise<boolean> => {
  return BcscCore.setToken(tokenType as number, token, expiry);
};
...
/**
 * Sets a user-created PIN for the specified account.
 ...
 */
export const setPIN = async (accountID: string, pin: string): Promise<import('./NativeBcscCore').PINSetupResult> => {
  return BcscCore.setPIN(accountID, pin);
};
...
export const verifyPIN = async (
  accountID: string,
  pin: string
): Promise<import('./NativeBcscCore').PINVerificationResult> => {
  return BcscCore.verifyPIN(accountID, pin);
};
 ...
 */
export const deletePIN = async (accountID: string): Promise<boolean> => {
  return BcscCore.deletePIN(accountID);
};
 ...
 */
export const hasPINSet = async (accountID: string): Promise<boolean> => {
  return BcscCore.hasPINSet(accountID);
};
...
export const setAccountSecurityMethod = async (
  ...
  return BcscCore.setAccountSecurityMethod(accountID, securityMethod);
};
 ...
 */
export const getAccountSecurityMethod = async (
  accountID: string
): Promise<import('./NativeBcscCore').AccountSecurityMethod> => {
  return BcscCore.getAccountSecurityMethod(accountID);
};
 ...
 */
export const isAccountLocked = async (accountID: string): Promise<import('./NativeBcscCore').AccountLockStatus> => {
  return BcscCore.isAccountLocked(accountID);
};
 ...
 */
export const setupDeviceSecurity = async (accountID: string): Promise<import('./NativeBcscCore').PINSetupResult> => {
  return BcscCore.setupDeviceSecurity(accountID);
};
...
export const unlockWithDeviceSecurity = async (
  ...
  return BcscCore.unlockWithDeviceSecurity(accountID, reason);
};
 ...
 */
export const isPINAutoGenerated = async (accountID: string): Promise<boolean> => {
  return BcscCore.isPINAutoGenerated(accountID);
};
...
/**
 * Complete secure state that can be hydrated after successful authentication.
 * This consolidates all PII and sensitive data from native secure storage.
 */
export interface HydratedSecureState {
  ...
  userCode?: string;
  /** Expiration time for device code (Unix timestamp) */
  deviceCodeExpiresAt?: number;

  // === PII ===
  /** User's birthdate (Unix timestamp) */
  birthdate?: number;
  ...
  // === Email Verification Flags (v3-compatible) ===
  ...
  /** User metadata for non-BCSC cards (name, address) */
  userMetadata?: SecureStateUserMetadata;
}

/**
 * Hydrates all secure state data after successful authentication.
 *
 * This is a convenience function that fetches all secure data in one call:
 * - Refresh token from secure token storage
 * - Registration access token from secure token storage
 * - Authorization request data (deviceCode, userCode, birthdate, serial, email, userMetadata)
 *
 * Call this after successful PIN verification or device security unlock to
 * populate the app's secure state.
 *
 * @returns A promise that resolves to the complete hydrated secure state
 *
 * @example
 * ```typescript
 * const unlockResult = await unlockWithDeviceSecurity(accountId);
 * if (unlockResult.success) {
 *   const secureState = await hydrateSecureState();
 *   if (secureState.success) {
 *     dispatch(setSecureState(secureState));
 *   }
 * }
 * ```
 */
export const hydrateSecureState = async (): Promise<HydratedSecureState> => {
  try {
    // Fetch all data in parallel for efficiency
    const [refreshTokenInfo, registrationTokenInfo, authRequest] = await Promise.all([
      getToken(TokenType.Refresh),
      getToken(TokenType.Registration),
      getAuthorizationRequest(),
    ]);

    const result: HydratedSecureState = {
      success: true,
    };

    // Extract tokens
    if (refreshTokenInfo?.token) {
      result.refreshToken = refreshTokenInfo.token;
    }
    if (registrationTokenInfo?.token) {
      result.registrationAccessToken = registrationTokenInfo.token;
    }

    // Extract authorization request data
    if (authRequest) {
      result.deviceCode = authRequest.deviceCode;
      result.userCode = authRequest.userCode;
      result.deviceCodeExpiresAt = authRequest.expiry;
      result.birthdate = authRequest.birthdate;
      result.serial = authRequest.csn;
      result.email = authRequest.verifiedEmail;

      // Build user metadata if any profile fields are present
      if (authRequest.firstName || authRequest.lastName || authRequest.middleNames || authRequest.address) {
        result.userMetadata = {
          firstName: authRequest.firstName,
          lastName: authRequest.lastName,
          middleNames: authRequest.middleNames,
          address: authRequest.address
            ? {
                streetAddress: authRequest.address.streetAddress,
                locality: authRequest.address.locality,
                postalCode: authRequest.address.postalCode,
                country: authRequest.address.country,
                region: authRequest.address.region,
              }
            : undefined,
        };
      }
    ...
    // Extract account flags
    const flags = await getAccountFlags();
    if (flags.isEmailVerified !== undefined) {
      result.isEmailVerified = flags.isEmailVerified as boolean;
    }
    if (flags.userSkippedEmailVerification !== undefined) {
      result.userSkippedEmailVerification = flags.userSkippedEmailVerification as boolean;
    }

    return result;
  } catch (error) {
    console.error('hydrateSecureState error:', error);
    return { success: false };
  }
};

/**
 * Clears all secure state data from native storage.
 *
 * This deletes:
 * - All tokens (access, refresh, registration)
 * - Authorization request data
 * - Account flags
 *
 * Call this on logout or account removal.
 *
 * @returns A promise that resolves to true if all data was cleared successfully
 */
export const clearSecureState = async (): Promise<boolean> => {
  try {
    await Promise.all([
    ...
    console.error('clearSecureState error:', error);
    return false;
  }
};
