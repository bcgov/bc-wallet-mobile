...
export interface Spec extends TurboModule {
  ...
  /**
   * Sets a user-created PIN for the account.
   * @returns Result containing success status and the PBKDF2 wallet key hash
   */
  setPIN(accountID: string, pin: string): Promise<PINSetupResult>;

  /**
   * Verifies a PIN for the specified account.
   * @returns Result with success status, lock info, and wallet key hash on success
   */
  verifyPIN(accountID: string, pin: string): Promise<PINVerificationResult>;
  deletePIN(accountID: string): Promise<boolean>;
  hasPINSet(accountID: string): Promise<boolean>;
  ...
  setAccountSecurityMethod(accountID: string, securityMethod: AccountSecurityMethod): Promise<boolean>;
  getAccountSecurityMethod(accountID: string): Promise<AccountSecurityMethod>;
  isAccountLocked(accountID: string): Promise<AccountLockStatus>;
  getBestAvailableAccountSecurityMethod(): Promise<AccountSecurityMethod>;
   ...
   * @returns Result containing success status and the PBKDF2 wallet key hash
   */
  setupDeviceSecurity(accountID: string): Promise<PINSetupResult>;
   ...
   * @returns Result with success status and wallet key hash on success
   */
  unlockWithDeviceSecurity(accountID: string, reason?: string): Promise<DeviceSecurityUnlockResult>;

  /**
   * Checks if the stored PIN was auto-generated (device security) or user-created.
   * Useful for prompting users to create their own PIN in future app versions.
   * @param accountID The account identifier
   * @returns true if PIN was auto-generated, false if user-created
   */
  isPINAutoGenerated(accountID: string): Promise<boolean>;
