...
class NativeCompatiblePinStorage(
    ...
    fun verifyPin(
        ...
        return try {
            ...
            val keyBytes = deriveKeyBytes(plainTextPin, secret.salt, algorithm)
            val base64Key = Base64.encodeToString(keyBytes, Base64.NO_WRAP)

            if (secret.key == base64Key) {
                return true
            }

            // Backward-compat migration for PINs created before #3974: those secrets stored
            // the key as ByteArray.contentToString() (e.g. "[-106, 10, ...]"). If the legacy
            // form matches, the PIN is correct, so re-save the same key/salt in Base64 so the
            // next unlock (and the wallet key from getPinHash) uses the new format.
            // Done inline rather than as a startup system check because PIN verification
            // runs before the STARTUP/MAIN_STACK system-check scopes.
            if (secret.key == keyBytes.contentToString()) {
                val migrated =
                    saveSecret(
                        NativePinSecret(
                            id = alias,
                            ...
                            algorithmUsed = algorithm,
                            key = base64Key,
                            isAutoGenerated = secret.isAutoGenerated,
                        ),
                    )
                if (migrated) {
    ...
    /**
     * Derives the raw PBKDF2 key bytes from the PIN using native-compatible parameters.
     * Callers apply the string encoding (see [deriveKey]).
     */
    ...
    /**
     * Derives a key from the PIN using PBKDF2, Base64-encoded to match iOS
     * (PINService.swift) so the wallet key string is stable across platforms.
     * contentToString() previously produced a Java byte-array debug string
     * (e.g. "[-106, 10, ...]") rather than a proper encoding.
     */
    private fun deriveKey(
        ...
        val keyBytes = deriveKeyBytes(plainTextPin, salt, algorithm)
     ...
     *
     * Self-migrates legacy-format keys to Base64 so every read path (user PIN
     * verification *and* device-security unlock, which never calls verifyPin)
     * converges on the cross-platform Base64 format. See migrateLegacyKeyIfNeeded.
     *
     * @param alias The alias/identifier for the PIN
    ...
    fun getPinHash(alias: String): Pair<String, Boolean>? =
        try {
            ...
            if (secret != null) {
                Pair(migrateLegacyKeyIfNeeded(secret), secret.isAutoGenerated)
            } else {
                null
            }
        } catch (e: Exception) {
    ...
    /**
     * Migrates a legacy-format PIN key to Base64, returning the Base64 form.
     *
     * Keys created before #3974 were stored as ByteArray.contentToString()
     * (e.g. "[-106, 10, -93, ...]"). The device-security unlock path reads the
     * key via getPinHash without ever calling verifyPin, so verifyPin's inline
     * migration can't reach it; this converges all read paths on Base64 and
     * needs no plaintext PIN since the legacy form round-trips back to bytes.
     *
     * "[" is not in the Base64 alphabet, so the prefix check has no false
     * positives on already-migrated keys. Returns the Base64 value even when
     * the re-save fails, so callers never observe the stale legacy string.
     */
    private fun migrateLegacyKeyIfNeeded(secret: NativePinSecret): String {
        val key = secret.key
        ...
        return try {
            ...
            val migrated =
                saveSecret(
                    NativePinSecret(
                        id = secret.id,
                        salt = secret.salt,
                        algorithmUsed = secret.algorithmUsed,
                        key = base64,
                        isAutoGenerated = secret.isAutoGenerated,
                    ),
                )
            ...
            Log.e(TAG, "Failed to parse legacy PIN key for alias: ${secret.id}; returning as-is", e)
            key
        }
    }
