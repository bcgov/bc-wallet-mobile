...
class BcscCoreModule(
    ...
    /**
     * Sets a PIN for the specified account.
     * Matches TypeScript: setPIN(accountID: string, pin: string): Promise<boolean>
     */
    @ReactMethod
    override fun setPIN(
        accountID: String,
     ...
     * Matches TypeScript: verifyPIN(accountID: string, pin: string): Promise<PINVerificationResult>
     ...
     * Matches TypeScript: deletePIN(accountID: string): Promise<boolean>
     */
    @ReactMethod
     ...
     * Matches TypeScript: hasPINSet(accountID: string): Promise<boolean>
     */
    @ReactMethod
    override fun hasPINSet(
        accountID: String,
        promise: Promise,
    ) {
        try {
        ...
        try {
            ...
            val updatedAccounts =
                existingAccounts.map { existingAccount ->
                    if (existingAccount.uuid == accountID) {
                        existingAccount.copy(accountSecurityType = accountSecurityType)
                    } else {
                        existingAccount
                    }
                }
     ...
     * Matches TypeScript: getAccountSecurityMethod(accountID: string): Promise<AccountSecurityMethod>
     */
    @ReactMethod
    override fun getAccountSecurityMethod(
        accountID: String,
        promise: Promise,
    ) {
        try {
            ...
            val account = existingAccounts.find { it.uuid == accountID }
            if (account == null) {
                promise.resolve("device_authentication") // Default
                return
            }
            ...
            Log.d(NAME, "getAccountSecurityMethod for account $accountID: $securityMethod")
    ...
    override fun isAccountLocked(
        ...
        try {
            ...
            Log.d(NAME, "isAccountLocked for account $accountID: locked=false (tracking not yet implemented)")
            promise.resolve(result)
        } catch (e: Exception) {
     ...
     * Matches TypeScript: setupDeviceSecurity(accountID: string): Promise<PINSetupResult>
     */
    @ReactMethod
    override fun setupDeviceSecurity(
        accountID: String,
        promise: Promise,
    ) {
        try {
     ...
     * Matches TypeScript: unlockWithDeviceSecurity(accountID: string, reason?: string): Promise<DeviceSecurityUnlockResult>
     */
    @ReactMethod
        ...
        try {
            ...
            val authRequest =
                NativeAuthorizationRequest(
                    ...
                    csn = data.getString("csn"),
                    verifiedEmail = data.getString("verifiedEmail"),
                    firstName = data.getString("firstName"),
                    lastName = data.getString("lastName"),
                    middleNames = data.getString("middleNames"),
                    issuer = data.getString("audience"),
                    requestStatus = NativeRequestStatus.fromValue(data.getInt("status")),
                    authorizationMethod = NativeAuthorizationMethod.fromValue(data.getInt("method")),
                    verificationOptions = data.getString("verificationOptions"),
                    verificationUriVideo = data.getString("verificationURIVideo"),
                    backCheckVerificationId = data.getString("backCheckVerificationId"),
                    evidenceUploadUri = data.getString("evidenceUploadURI"),
                    expiry = data.getDouble("expiry").takeIf { !it.isNaN() }?.let { Date((it * 1000).toLong()) },
                    authorizationExpiry =
    ...
    override fun setCredential(
        ...
        try {
            ...
            val providerJson = sharedPreferences.getString("provider", null)
            val providerData = if (providerJson != null) {
            ...
            val clientRegistration = if (providerData.has("clientRegistration")) {
            ...
            while (iterator.hasNextKey()) {
                ...
                when (val value = credentialData.getDynamic(key)) {
                    null -> credentialJson.put(key, JSONObject.NULL)
                    else -> {
                        when (value.type) {
