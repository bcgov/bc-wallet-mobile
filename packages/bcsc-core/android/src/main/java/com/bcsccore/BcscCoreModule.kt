...
class BcscCoreModule(
  ...
  @ReactMethod
  override fun decodeLoginChallenge(jwt: String, key: ReadableMap?, promise: Promise) {
    try {
      if (jwt.isBlank()) {
      ...
      val verified = if (key != null) {
        verifyJwtSignature(signedJWT, key)
      } else {
        false
      }
      
      val claims = Arguments.createMap().apply {
        ...
        putDouble("exp", jwtClaims.expirationTime?.time?.div(1000)?.toDouble() ?: 0.0)
        putString("bcsc_client_name", jwtClaims.getStringClaim("bcsc_client_name") ?: "")
        putDouble("iat", jwtClaims.issueTime?.time?.div(1000)?.toDouble() ?: 0.0)
   ...
   */
  private fun verifyJwtSignature(signedJWT: SignedJWT, jwk: ReadableMap): Boolean {
    return try {
