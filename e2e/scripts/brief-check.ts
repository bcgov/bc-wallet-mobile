...
function selfTest(): void {
  ...
  const migrationIos = cellOf(model, 'ext-migration-v3', 'ios')
  assert.deepEqual(
  ...
  const unlockIos = cellOf(model, 'j-migration-v4-unlock', 'ios')
  ...
  assert.equal(cellOf(model, 'j-migration-v3-add-card', 'ios').status, 'pass')
