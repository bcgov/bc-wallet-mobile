 ...
 *
 * Progress logs go to STDERR; the only STDOUT output is dotenv-style `KEY=VALUE` lines — the
 * resolved ids and base URL, never the tenant creds — so CI can append them directly:
 * `./node_modules/.bin/tsx scripts/issuer-provision.ts >> "$GITHUB_ENV"`. The same id lines are
 * written to e2e/issuer/.issuer-env for src/api/issuer.ts to pick up locally.
 */
...
function emit(config: IssuerConfig, did: string, schemaId: string, credDefId: string): void {
  ...
  // Ids only — never the tenant creds, so stdout stays secret-free wherever it is redirected.
  // Whoever runs this already has them: CI via 1Password export-env, a local run via .env.e2e.
  console.log([`ISSUER_BASE_URL=${config.baseUrl}`, ...idLines].join('\n'))
}
