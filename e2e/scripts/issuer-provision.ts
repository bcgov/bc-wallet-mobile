...
function emit(config: IssuerConfig, did: string, schemaId: string, credDefId: string): void {
  ...
  // stdout carries everything the CI test step needs (>> $GITHUB_ENV), creds included.
  console.log(
    [`ISSUER_BASE_URL=${config.baseUrl}`, `ISSUER_TENANT_ID=${config.tenantId}`, `ISSUER_API_KEY=${config.apiKey}`]
      .concat(idLines)
      .join('\n')
  )
}
