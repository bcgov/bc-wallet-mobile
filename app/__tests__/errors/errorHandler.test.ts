...
import { ErrorRegistry, ErrorCategory, ErrorSeverity } from '../../src/errors/errorRegistry'
...
describe('errorHandler', () => {
  ...
  describe('emitError', () => {
    ...
    it('should track error in analytics when showing modal', () => {
      ...
      expect(Analytics.trackAlertDisplayEvent).toHaveBeenCalledWith(
        ErrorRegistry.CAMERA_BROKEN.alertEvent
      )
    })
  ...
  describe('emitBifoldError', () => {
    it('should emit a raw BifoldError', () => {
      ...
      expect(appLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('BifoldError:1234')
      )
      expect(DeviceEventEmitter.emit).toHaveBeenCalledWith(
        EventTypes.ERROR_ADDED,
        bifoldError
  ...
  describe('trackErrorAction', () => {
    it('should track error action in analytics', () => {
      ...
      expect(Analytics.trackAlertActionEvent).toHaveBeenCalledWith(
        ErrorRegistry.CAMERA_BROKEN.alertEvent,
        'dismiss'
      )
      expect(appLogger.debug).toHaveBeenCalledWith(
        expect.stringContaining(AlertInteractionEvent.ALERT_ACTION)
      )
    })
    ...
    it('should log warning for unknown error keys', () => {
      ...
      expect(appLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Unknown error key for tracking')
      )
