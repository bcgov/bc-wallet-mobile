...
import useBCSCSecureActions from '../../hooks/useBCSCSecureActions'
  ...
  const cancelVerificationRequest = useCallback(
    async (verificationRequestId: string): Promise<any> => {
      return withAccount(async (account) => {
        ...
        secureActions.updateVerificationRequest(undefined, undefined)

        return data
      })
    },
    [_getDeviceCode, apiClient, secureActions]
  )
