...
import { AccountSecurityMethod, canPerformDeviceAuthentication, getAccount, setPIN } from 'react-native-bcsc-core'
  ...
  const validateAndContinue = useCallback(
    async (pin1: string, pin2: string) => {
      try {
        ...
        if (!account?.id) {
          setErrorMessage2('Error creating account')
          setLoading(false)
          return
        }
        const result = await setPIN(account.id, pin1)
    ...
    [checked, logger, updateWalletKey, dispatch, register, hydrateSecureState]
  )
