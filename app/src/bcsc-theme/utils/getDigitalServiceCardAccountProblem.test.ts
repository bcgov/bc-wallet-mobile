...
import { getDigitalServiceCardAccountProblem } from './getDigitalServiceCardAccountProblem'
...
describe('getDigitalServiceCardAccountProblem', () => {
  it('returns "suspended" for a 400 with a "suspended" error_description', () => {
    ...
    expect(getDigitalServiceCardAccountProblem(error)).toBe('suspended')
  })
  ...
  it('returns "deactivated" for a 400 with a "deactivated" error_description', () => {
    ...
    expect(getDigitalServiceCardAccountProblem(error)).toBe('deactivated')
  })
  ...
  it('returns undefined when error is not "unauthorized_client", even if error_description mentions suspended/deactivated', () => {
    ...
    expect(getDigitalServiceCardAccountProblem(error)).toBeUndefined()
  })
  ...
  it('returns undefined for a plain (non-AppError) error', () => {
    expect(getDigitalServiceCardAccountProblem(new Error('plain error'))).toBeUndefined()
  })
})
