...
export class KeyRotationSystemCheck implements SystemCheckStrategy {
  ...
  async runCheck(): Promise<boolean> {
    ...
    // Keys without a usable timestamp are left out rather than blocking the check: signing uses
    // the metadata-newest key, so an untracked key (Android reports createdAt 0 for it, which
    // would otherwise read as a 1970 key) is never the active one whose age rotation is about.
