...
import { AccountFlags } from 'react-native-bcsc-core'
...
export interface BCSCState {
  ...
  // TODO (bm): use value from ID token response, not just user selection
 ...
 *
 * In v3 native apps:
 * - iOS: AuthorizationRequest (containing deviceCode, userCode, birthdate, csn, expiry)
 *   is persisted via NSKeyedArchiver to Application Support directory
 * - Android: AuthorizationRequest is stored as part of Provider via MAFileStorage
 *   with AES-GCM encryption (Android KeyStore)
 * - Both: Tokens stored in Keychain (iOS) / encrypted file storage (Android)
 *
 * For v4 like-for-like behavior:
 * - This state will be persisted to native secure storage (not AsyncStorage)
 * - Hydrated on app launch after successful authentication
 * - Cleared on account removal / logout
 *
 * Storage locations:
 * - iOS: NSKeyedArchiver file (AuthorizationRequest) + Keychain (tokens)
 * - Android: AES-GCM encrypted files via MAFileStorage
 */
export interface BCSCSecureState {
  /** Whether secure state has been loaded from native storage */
  isHydrated: boolean

  /** Account verification status - determined by credential presence in v3 compatibility mode */
  verified?: boolean

  // === Tokens (stored in native KeychainTokenStorageService / NativeCompatibleStorage) ===
  /** OAuth refresh token for API authentication */
  refreshToken?: string
  /** Registration access token for DCR updates */
  registrationAccessToken?: string

  // === Device Authorization (persisted in v3 as part of AuthorizationRequest) ===
  ...
  // === PII from AuthorizationRequest (persisted in v3) ===
  ...
  // === Non-BCSC User Metadata (for alternative ID verification) ===
  /** Metadata for users verifying with non-BCSC cards (name, address) */
  userMetadata?: NonBCSCUserMetadata

  // === Account Flags (v3 compatible storage) ===
  /** Account flags like isEmailVerified, userSkippedEmailVerification, etc. */
  accountFlags?: AccountFlags

  // === Verification Data (from v3 AuthorizationRequest) ===
  ...
  // === Evidence Data (from v3 EvidenceMetadata storage) ===
...
enum BCSCDispatchAction {
  ...
  UPDATE_SECURE_EMAIL_CONFIRMED = 'bcsc/updateSecureEmailConfirmed',
  ...
  UPDATE_SECURE_ACCOUNT_FLAGS = 'bcsc/updateSecureAccountFlags',
  CLEAR_SECURE_ACCOUNT_FLAGS = 'bcsc/clearSecureAccountFlags',
...
const bcReducer = (state: BCState, action: ReducerAction<BCDispatchAction>): BCState => {
  switch (action.type) {
    ...
    case BCSCDispatchAction.UPDATE_SECURE_EMAIL_CONFIRMED: {
      const emailConfirmed = (action?.payload || []).pop() ?? false
      const bcscSecure = { ...state.bcscSecure, emailConfirmed }
      return { ...state, bcscSecure }
    }
    ...
    case BCSCDispatchAction.UPDATE_SECURE_ACCOUNT_FLAGS: {
      const accountFlags = (action?.payload || []).pop() ?? {}
      const bcscSecure = { ...state.bcscSecure, accountFlags }
      return { ...state, bcscSecure }
    }
    case BCSCDispatchAction.CLEAR_SECURE_ACCOUNT_FLAGS: {
      const bcscSecure = { ...state.bcscSecure, accountFlags: {} }
      return { ...state, bcscSecure }
    }
