...
export const useSetupSteps = (store: BCState): SetupStepsResult => {
  ...
  return useMemo(() => {
    ...
    const emailConfirmed = Boolean(store.bcscSecure.emailConfirmed)
    ...
    const step5Completed = Boolean(store.bcscSecure.verified || store.bcscSecure.accountFlags?.userSubmittedVerificationVideo)
